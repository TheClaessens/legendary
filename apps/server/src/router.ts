...
import { createInitialState, flipVillainDeck, playCard, recruitHero, fightVillain, fightMastermind, hydrateState } from "@legendary/game-engine";
  ...
  game: router({
    ...
    recruitHero: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        let newState;
        try {
          newState = recruitHero(state, input.hqSlot);
        } catch (err) {
          throw new TRPCError({ code: "BAD_REQUEST", message: (err as Error).message });
        }
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    ...
    fightVillain: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        let newState;
        try {
          newState = fightVillain(state, input.cityIndex);
        } catch (err) {
          throw new TRPCError({ code: "BAD_REQUEST", message: (err as Error).message });
        }
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    ...
    fightMastermind: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        let newState;
        try {
          newState = fightMastermind(state);
        } catch (err) {
          throw new TRPCError({ code: "BAD_REQUEST", message: (err as Error).message });
        }
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
